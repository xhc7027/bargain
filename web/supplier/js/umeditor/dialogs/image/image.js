...
//                }
        ...
        callback: function (editor, $w, url, state) {
            ...
            if (state == "SUCCESS") {
                //显示图片计数+1
                Upload.showCount++;
                //console.log("link："+editor.options.imagePath);
                //console.log("url:"+url);
        ...
        submit: function (callback) {
            ...
            $(me.dialog).delegate( ".edui-image-file", "change", function ( e ) {
               ...
               //$(this).parent()[0].submit();
                ...
                var file = $(this).parent().find("input").get(0).files[0];
                //图片上传
                /**/
                idouziCos.upimages(file,function(res){
                	if(res.status == 1){
                		var r = {originalName:file.name,name:null,url:res.data.data.download_url,size:file.size,type:idouziCos.getFileExt(file.name),state:"SUCCESS"};
                		if(res.data.code != 0){
                			r.state = res.data.message;
                		}
                	}else{
                		var r = {};
                		r.state = res.message;
                	...
                	r = JSON.stringify(r);
                	//console.log(r);
                	//{"originalName":"0.png","name":null,"url":"http:\/\/idouziimg-10006892.image.myqcloud.com\/20151214\/20151214142751_34480..png","size":1487,"type":".png","state":"SUCCESS"}
                	if(r == '')return;
                    me.uploadComplete(r);
                    //$(this).unbind('load');
                    //$(this).remove();
                ...
                me.dialog.find('.edui-image-content').on('drop',function (e) {
                    ...
                    $.each(fileList, function (i, f) {
                        if (/^image/.test(f.type)) {
                        	//console.log(f);
                        	//cos上传
                        	idouziCos.upimages(f,function(res){
                            	if(res.status == 1){
                            		var r = {originalName:f.name,name:null,url:res.data.data.download_url,size:f.size,type:idouziCos.getFileExt(f.name),state:"SUCCESS"};
                            		if(res.data.code != 0){
                            			r.state = res.data.message;
                            		}
                            	}else{
